...
def main(anchors = None,labels = None, model_addr="/sd/demo.kmodel", sensor_window=input_size, lcd_rotation=0, sensor_hmirror=False, sensor_vflip=False):
    ...
    global num,begin_flag,task_flag,red_thresholds,red_flag,condi_flag,num1,num2,send_num_flag1,send_num_flag2,send_num_flag,list,delay_num
    ...
    while(True):
        ...
        #task1
        if task_flag == 'a':      
            kpu.init_yolo2(task, 0.8, 0.3, 5, anchors) # threshold:[0,1], nms_value: [0, 1]
            img.draw_string(0, 0, "task1", color=(255, 0, 0), scale=2)
            img.draw_rectangle((0,60,500,35))
            #print("正在执行task1")
            ...
            if objects:
                ...
                for obj in objects:
                    ...
                    #K210找十字路口(已调)
                    img.draw_rectangle((0,60,500,35))
                    red_blobs_crossroad = img.find_blobs([(0, 100, 12, 127, -124, 117)],roi=(0,60,500,35),x_stride=10, y_stride=10, pixels_threshold=2000)
