                        ...
                        if num == 15:
                            ...
                            if most_commom_value != None:#判断是否返回最大值
                                if begin_flag == 1:
                                    ...
                                    print("#0b"+str(most_commom_value))
                            else:
                                print("no most_commom_value")
                                img.draw_string(0, 150, "none_finish" , scale=2, color=(255, 255, 255))
                #K210找十字路口                
                red_blobs_crossroad = img.find_blobs([red_thresholds],roi=roi_1,x_stride=10, y_stride=10, pixels_threshold=1000)
                    ...
                    for obj in objects:
                        ...
                        if num1_center_x is not None and num1_center_y is not None and num2_center_x is not None and num2_center_y is not None:
                            if num1_center_x < num2_center_x:
                                task3_num_flag1 = str("#0l") + str(num1)#  #0l3
                                task3_num_flag2 = str("#0r") + str(num2)#  #0r4
                            else:
                                task3_num_flag1 = str("#0r") + str(num1)#  #0r3
                                task3_num_flag2 = str("#0l") + str(num2)#  #0l4
                            task3_num_flag = task3_num_flag1+ task3_num_flag2
                            task3_num_flag  = task3_num_flag[:4]+task3_num_flag[6:]#如 #0l3r4;
                            uart.write(send_num_flag)
                            uart.write(task3_num_flag)
                            print("task3_num_flag:%s"%task3_num_flag)           
            lcd.display(img)
