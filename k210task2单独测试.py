...
def main(anchors = None,labels = None, model_addr="/sd/demo.kmodel", sensor_window=input_size, lcd_rotation=0, sensor_hmirror=False, sensor_vflip=False):
    ...
    global num,begin_flag,red_thresholds,red_flag,condi_flag,send_task2_flag,img_condi,datapack_l,datapack_r,datapack,receive_task,anchor
    ...
    try:
        ...
        while(True):
            ...
            print("正在执行task2")
            #由红色色块去设置roi
            red_blob = find_red_blob(img,red_thresholds)
            #roi_left = (0,0,red_blob[0],240)
            #roi_right= (red_blob[0]+red_blob[2],0,320-red_blob[0]-red_blob[2],240)
            roi_left = (0,0,10,10)
            roi_right= (10,0,10,10)
            #画出roi
            img.draw_rectangle(roi_left,color=(0,0,255))
            img.draw_rectangle(roi_right,color=(0,0,255))
            #图像分割
            img_left = img.crop((0,0,10,10),1,1,False)#参数可能得改 img.crop(roi, x_scale, y_scale, copy_to_fb)
            img_right = img.crop((0,0,10,10),1,1,False)#如果copy_to_fb为True，则裁剪后的图像将被复制到帧缓冲区（framebuffer）。帧缓冲区通常比MicroPython堆（heap）中的内存要大，因此可以容纳更大的图像
            #img_left = img_left.resize(224,224)
            #img_right = img_right.resize(224,224)
            img_left = img.copy(roi = (0,0,300,240))

            #切换左右图
            if img_condi == "img_left":
                kpu.init_yolo2(anchor, anchor_num=10, img_w=300, img_h=240, net_w=300 , net_h=240 ,layer_w=10 ,\
layer_h=8, threshold=0.5, nms_value=0.2, classes=8)
                objects = kpu.run_yolo2(task, img_left)
                condi_flag  = "#####0l"
                img_condi = "img_right"
                send_task2_flag += 1
            elif img_condi == "img_right":
                objects = kpu.run_yolo2(task, img_right)
                condi_flag  = "#####0r"    #这里可能需要改成#0r
                img_condi = "img_left"
                send_task2_flag += 1
